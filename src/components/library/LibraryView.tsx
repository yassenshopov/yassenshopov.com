...
function toOccurrences(items: LibraryItem[]): LibraryOccurrence[] {
  ...
  for (const item of items) {
    ...
    const latestTimestamp = Math.max(...entries.map(entryTimestamp));
    ...
    for (const entry of entries) {
      ...
      out.push({
        ...
        yearKey,
        totalEntries,
        isLatestEntry: entryTimestamp(entry) === latestTimestamp,
      });
    }
  }
...
export function LibraryView({ items }: { items: LibraryItem[] }) {
  ...
  const overallStats = useMemo(() => getStatistics(allItems), [getStatistics, allItems]);

  const { bookCount, movieCount, seriesCount } = useMemo(() => {
    let books = 0;
    let movies = 0;
    let series = 0;
    ...
    return { bookCount: books, movieCount: movies, seriesCount: series };
  }, [allItems]);
  ...
  const heroCovers = useMemo(
    () => allItems.map((item) => item.coverImage).filter((src): src is string => Boolean(src)),
    [allItems]
  );
  ...
  const { visibleByYear, visibleYearKeys } = useMemo(() => {
    const grouped = groupOccurrencesByYear(toOccurrences(sortedItems));
    return { visibleByYear: grouped, visibleYearKeys: sortYearKeys(Object.keys(grouped)) };
  }, [sortedItems]);

  // Build ToC sections from the full filtered list so users can jump to a
  // year even before infinite scroll has loaded it.
  const tocSections: LibraryTocSection[] = useMemo(() => {
    ...
    // labelForYear only depends on `category` (via undatedLabel).
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [allSortedItems, category]);
